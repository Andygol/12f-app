...
La "configurazione" di un'applicazione è tutto quello che può variare da un [deployment](./codebase.md) all'altro (staging, production, ambienti di sviluppo). Ad esempio:

* Valori da usare per connettersi a un database, Memcached, oppure altri [backing service](./backing-services.md);
